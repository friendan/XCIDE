...
static const uint64_t X_S512K[80] = {
...
#define X_S512F(x,y,z) (z ^ (x & (y ^ z)))
#define X_S512G(x,y,z) (((x | y) & z) | (x & y))
#define X_S512H(x) (ROR64(x, 28) ^ ROR64(x, 34) ^ ROR64(x, 39))
#define X_S512I(x) (ROR64(x, 14) ^ ROR64(x, 18) ^ ROR64(x, 41))
#define X_S512J(x) (ROR64(x, 1) ^ ROR64(x, 8) ^ SHR64(x, 7))
#define X_S512L(x) (ROR64(x, 19) ^ ROR64(x, 61) ^ SHR64(x, 6))

#define Sha512Round( a, b, c, d, e, f, g, h, i)     \
    t0 = h + X_S512I(e) + X_S512F(e, f, g) + X_S512K[i] + W[i];             \
    t1 = X_S512H(a) + X_S512G(a, b, c);                            \
    d += t0;                                           \
    h = t0 + t1;

X_CSHA512::X_CSHA512(void)
{
...
void X_CSHA512::SetBaseData(unsigned char* pBaseDataIn)
{
	memcpy(BaseData, pBaseDataIn, 64);
}

void X_CSHA512::GetBaseData(unsigned char* pBaseDataOut)
{
	for (int i = 0; i < 8; i++)
	{
...
void X_CSHA512::SetCalcData(unsigned char* pCalcDataIn)
{
	memcpy(CalcData, pCalcDataIn, 128);
}

bool X_CSHA512::InitHash(unsigned char* pBaseData /* = NULL */)
{
...
bool X_CSHA512::CalcHash(unsigned char* pBufferIn, int BufferInSize, unsigned char* pDigestOut)
{
...
void X_CSHA512::Sha512Calc()
{
	...
	for (int i = 16; i < 80; i++)
	{
		W[i] = X_S512L(W[i - 2]) + W[i - 7] + X_S512J(W[i - 15]) + W[i - 16];
	}
