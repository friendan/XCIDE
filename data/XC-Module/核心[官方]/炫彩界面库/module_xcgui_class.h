...
{
public:
	virtual HXCGUI GetHXCGUI()=0;
	CXBase(){}
//@备注 获取对象最终类型.  
//@返回 返回对象类型.
//@别名  取类型()
	XC_OBJECT_TYPE GetType(){return XObj_GetType(GetHXCGUI());}
//@备注 获取对象的基础类型,  
//XC_ERROR, XC_WINDOW, XC_ELE, XC_SHAPE, XC_ADAPTER
//@返回 返回对象类型, 以下类型之一:
//@别名  取基础类型()
	XC_OBJECT_TYPE GetTypeBase(){return XObj_GetTypeBase(GetHXCGUI());}
//@备注 获取对象扩展类型.  
//@返回 返回对象扩展类型.
//@别名  取类型扩展()
	XC_OBJECT_TYPE_EX GetTypeEx(){return XObj_GetTypeEx(GetHXCGUI());}
//@备注 如果是按钮, 请使用按钮的增强接口 XBtn_SetTypeEx()
//@参数 nType 扩展类型,参见宏定义.
//@别名  置类型扩展()
	void SetTypeEx(XC_OBJECT_TYPE_EX nType){XObj_SetTypeEx(GetHXCGUI(), nType);}
};
//@备注  继承: CXBase
//@别名  可视对象类
...
//@参数 nStyle 样式值.
//@别名  置样式()
	void SetStyle(XC_OBJECT_STYLE nStyle){XUI_SetStyle(GetHXCGUI(), nStyle);}
//@备注 获取UI对象样式
//@返回 返回UI对象样式.
//@别名  取样式()
	XC_OBJECT_STYLE GetStyle(){return XUI_GetStyle(GetHXCGUI());}
//@备注 启用或禁用样式, 并且覆盖内嵌子元素属性,  例如:滚动视图上的滚动条, 滚动条上的按钮
//@参数 bEnable 是否启用.
//@别名  启用CSS()
	void EnableCSS(BOOL bEnable){XUI_EnableCSS(GetHXCGUI(), bEnable);}
//@备注 启用或禁用样式, 仅设置自身属性, 不包含内嵌子元素属性,  例如:滚动视图上的滚动条, 滚动条上的按钮
//@参数 bEnable 是否启用.
//@别名  启用CSS扩展()
	void EnableCssEx(BOOL bEnable){XUI_EnableCssEx(GetHXCGUI(), bEnable);}
//@备注 设置CSS[套用样式]名称
//@参数 pName 套用样式名称.
//@别名  置CSS名称()
	void SetCssName(const wchar_t* pName){XUI_SetCssName(GetHXCGUI(), pName);}
//@备注 获取CSS样式名称
//@返回 返回CSS样式名称.
//@别名  取CSS名称()
	const wchar_t* GetCssName(){return XUI_GetCssName(GetHXCGUI());}
};
//@分组}
//@分组{ 窗口
...
public:
	...
	virtual HXCGUI GetHXCGUI(){ return m_hWindow; }
	operator HWINDOW() const { return m_hWindow; }
	operator HXCGUI(){ return m_hWindow; }
	CXWindow(){}
//@备注 创建窗口  
//@参数 x 窗口左上角x坐标.
//@参数 y 窗口左上角y坐标.
...
//@参数 bEnable 是否启用
//@别名  启用限制窗口大小()
	void EnableLimitWindowSize(BOOL bEnable){XWnd_EnableLimitWindowSize(m_hWindow, bEnable);}
//@参数 bEnable 是否启用
//@别名  启用拖放文件()
...
//@别名  取内填充大小()
	void  GetPadding(__out paddingSize_* pPadding){ XWnd_GetPadding(m_hWindow, pPadding);}
//@备注 设置窗口拖动边框大小.  
//@参数 left 窗口左边大小.
//@参数 top 窗口上边大小.
//@参数 right 窗口右边大小.
...
//@参数 nDPI DPI值
//@别名  置DPI()
	void SetDPI(int nDPI){XWnd_SetDPI(m_hWindow, nDPI);}
//@备注 获取当前窗口所在显示器DPI
//@返回 返回窗口DPI
//@别名  取DPI()
	int GetDPI(){return XWnd_GetDPI(m_hWindow);}
//@参数 hImage 图标句柄
//@别名  置图标()
...
//@别名  销毁()
	void DestroyWindow(){XWnd_DestroyWindow(m_hWindow);}
...
//@返回 如果成功返回TRUE,否则返回FALSE
//@别名  取光标位置()
	BOOL GetCursorPos(POINT* pPt){return XWnd_GetCursorPos(m_hWindow, pPt);}
//@备注 封装的系统API: ClientToScreen(), 内部做了DPI适配
//@参数 pPt 接收返回坐标点
//@返回 如果成功返回TRUE,否则返回FALSE
//@别名  客户区坐标点到屏幕()
	BOOL ClientToScreen(POINT* pPt){return XWnd_ClientToScreen(m_hWindow, pPt);}
//@备注 封装的系统API: ScreenToClient(), 内部做了DPI适配
//@参数 pPt 接收返回坐标点
//@返回 如果成功返回TRUE,否则返回FALSE
//@别名  屏幕坐标点到客户区()
	BOOL ScreenToClient(POINT* pPt){return XWnd_ScreenToClient(m_hWindow, pPt);}
//@备注 窗口客户区坐标转换到缩放后DPI坐标
//@参数 pRect 接收返回坐标
//@别名  坐标转换DPI()
	void RectToDPI(RECT* pRect){XWnd_RectToDPI(m_hWindow, pRect);}
//@备注 窗口客户区坐标点转换到缩放后
//@参数 pPt 接收返回坐标点
//@别名  坐标点转换DPI()
	void PointToDPI(POINT* pPt){XWnd_PointToDPI(m_hWindow, pPt);}
//@参数 pRect 坐标
//@别名  取坐标()
...
//@别名  置顶()
	void SetTop(BOOL bTop = TRUE){XWnd_SetTop(m_hWindow, bTop);}
//@参数 bMaximize 是否最大化
//@别名  最大化()
...
//@返回 如果成功返回TRUE,否则返回FALSE
//@别名  置窗口位置()
	BOOL SetWindowPos(HWND hWndInsertAfter, int X, int Y, int cx, int cy, UINT uFlags){return XWnd_SetWindowPos(m_hWindow, hWndInsertAfter, X, Y, cx, cy, uFlags);}
//@备注 设置窗口定时器.  
//@参数 nIDEvent 定时器ID.
//@参数 uElapse 间隔值,单位毫秒.
//@返回 参见MSDN.
...
//@参数 pRect 返回坐标.
//@别名  取主视图坐标()
	void GetViewRect(RECT* pRect){XFrameWnd_GetViewRect(m_hWindow, pRect);}
//@备注 设置主视图元素.  
//@参数 hEle 元素句柄.
//@别名  置视图()
...
//@参数 nWidth 宽度
//@别名  置窗格分隔条宽度()
	void SetPaneSplitBarWidth(int nWidth){XFrameWnd_SetPaneSplitBarWidth(m_hWindow, nWidth);}
//@返回 返回格分隔条宽度
//@别名  取窗格分隔条宽度()
	int GetPaneSplitBarWidth(){return XFrameWnd_GetPaneSplitBarWidth(m_hWindow);}
//@备注 设置窗格组TabBar高度
//@参数 nHeight 高度
//@别名  置TabBar条高度()
...
//@参数 bottom 底边间隔
//@别名  置布局外间距()
	void SetLayoutMargin(int left, int top, int right, int bottom){XFrameWnd_SetLayoutMargin(m_hWindow, left, top, right, bottom);}

//@备注 获取框架窗口停靠码头元素, 若是没有返回空
//@参数 number  码头编号, 1:左侧码头, 2:顶部码头, 3:右侧码头, 4:底部码头
//@返回 返回码头句柄, 类型为基础元素
//@别名 取码头()
	HELE GetDock(int number){ return XFrameWnd_GetDock(m_hWindow, number);}
...
//@别名  是否显示()
	BOOL IsShow(){return XWidget_IsShow(GetHXCGUI());}
//@参数 bShow 是否显示
//@别名  显示()
	void Show(BOOL bShow){XWidget_Show(GetHXCGUI(), bShow);}
//@备注 该对象是否受布局控制
//@参数 bEnable 是否启用
//@别名  启用布局控制()
	void EnableLayoutControl(BOOL bEnable){XWidget_EnableLayoutControl(GetHXCGUI(), bEnable);}
//@返回 如果成功返回TRUE,否则返回FALSE
//@别名  是否布局控制()
	BOOL IsLayoutControl(){return XWidget_IsLayoutControl(GetHXCGUI());}
//@返回 元素句柄.
//@别名  取父元素()
	HELE GetParentEle(){return XWidget_GetParentEle(GetHXCGUI());}
//@备注 获取父对象,父可能是元素或窗口,通过此函数可以检查是否有父
//@返回 对象句柄.
//@别名  取父句柄()
	HXCGUI GetParent(){return XWidget_GetParent(GetHXCGUI());}
//@备注 获取HWND句柄
//@返回 HWND句柄.
//@别名  取窗口句柄系统()
	HWND GetHWND(){return XWidget_GetHWND(GetHXCGUI());}
//@备注 获取HWINDOW句柄
//@返回 HWINDOW句柄.
//@别名  取窗口句柄()
	HWINDOW GetHWINDOW(){return XWidget_GetHWINDOW(GetHXCGUI());}
//@备注 设置元素ID
//@参数 nID ID值.
//@别名  置ID()
	void SetID(int nID){XWidget_SetID(GetHXCGUI(), nID);}
//@备注 获取元素ID
//@返回 返回元素ID.
//@别名  取ID()
	int GetID(){return XWidget_GetID(GetHXCGUI());}
//@备注 设置元素UID, 全局唯一标识符
//@参数 nUID UID值.
//@别名  置UID()
	void SetUID(int nUID){XWidget_SetUID(GetHXCGUI(), nUID);}
//@备注 获取元素UID,全局唯一标识符
//@返回 返回元素UID.
//@别名  取UID()
	int GetUID(){return XWidget_GetUID(GetHXCGUI());}
//@备注 设置元素name
//@参数 pName name值,字符串指针.
//@别名  置名称()
	void SetName(const wchar_t* pName){XWidget_SetName(GetHXCGUI(), pName);}
//@备注 获取元素name
//@返回 返回name.
//@别名  取名称()
	const wchar_t* GetName(){return XWidget_GetName(GetHXCGUI());}
//@备注 强制换行
//@参数 bWrap 是否换行
//@别名  布局项_启用换行()
	void LayoutItem_EnableWrap(BOOL bWrap){XWidget_LayoutItem_EnableWrap(GetHXCGUI(), bWrap);}
//@备注 根据水平垂直布局变换, 交换属性(宽度,高度,最小宽度,最小高度)
//@参数 bEnable 是否启用
//@别名  布局项_启用交换()
	void LayoutItem_EnableSwap(BOOL bEnable){XWidget_LayoutItem_EnableSwap(GetHXCGUI(), bEnable);}
//@备注 向反方向对齐
//@参数 bFloat 是否浮动
//@别名  布局项_启用浮动()
	void LayoutItem_EnableFloat(BOOL bFloat){XWidget_LayoutItem_EnableFloat(GetHXCGUI(), bFloat);}
//@参数 nType 类型
//@参数 nWidth 宽度
//@别名  布局项_置宽度()
	void LayoutItem_SetWidth(layout_size_ nType, int nWidth){XWidget_LayoutItem_SetWidth(GetHXCGUI(), nType, nWidth);}
//@参数 nType 类型
//@参数 nHeight 高度
//@别名  布局项_置高度()
	void LayoutItem_SetHeight(layout_size_ nType, int nHeight){XWidget_LayoutItem_SetHeight(GetHXCGUI(), nType, nHeight);}
//@参数 pType 类型
//@参数 pWidth 返回值
//@别名  布局项_取宽度()
	void LayoutItem_GetWidth(layout_size_* pType, int* pWidth){XWidget_LayoutItem_GetWidth(GetHXCGUI(), pType, pWidth);}
//@参数 pType 类型
//@参数 pHeight 返回值
//@别名  布局项_取高度()
	void LayoutItem_GetHeight(layout_size_* pType, int* pHeight){XWidget_LayoutItem_GetHeight(GetHXCGUI(), pType, pHeight);}
//@备注 根据水平垂直轴变化对齐
//@参数 nAlign 对齐方式
//@别名  布局项_置对齐()
	void LayoutItem_SetAlign(layout_align_axis_ nAlign){XWidget_LayoutItem_SetAlign(GetHXCGUI(), nAlign);}
//@参数 left 左边大小
//@参数 top 上边大小
//@参数 right 右边大小
//@参数 bottom 下边大小
//@别名  布局项_置外间距()
	void LayoutItem_SetMargin(int left, int top, int right, int bottom){XWidget_LayoutItem_SetMargin(GetHXCGUI(), left, top, right, bottom);}
//@参数 pMargin 接收返回
//@别名  布局项_取外间距()
	void LayoutItem_GetMargin(marginSize_* pMargin){XWidget_LayoutItem_GetMargin(GetHXCGUI(), pMargin);}
//@备注 限制大小仅针对缩放有效(自动, 填充父, 比例, 百分比)
//@参数 width 最小宽度
//@参数 height 最小高度
//@别名  布局项_置最小大小()
	void LayoutItem_SetMinSize(int width, int height){XWidget_LayoutItem_SetMinSize(GetHXCGUI(), width, height);}
//@备注 相对位置, 值大于等于0有效
//@参数 left 左边距离
//@参数 top 上边距离
//@参数 right 右边距离
//@参数 bottom 下边距离
//@别名  布局项_置位置()
	void LayoutItem_SetPosition(int left, int top, int right, int bottom){XWidget_LayoutItem_SetPosition(GetHXCGUI(), left, top, right, bottom);}
};
...
//@参数 pRect 接收返回坐标
//@别名  客户区坐标到窗口客户区DPI()
	void RectClientToWndClientDPI(RECT* pRect){XEle_RectClientToWndClientDPI(m_hEle, pRect);}
//@备注 元素客户区坐标转换到窗口客户区坐标.  
//@参数 pPt 接收返回坐标点.
//@别名  客户区点到窗口客户区()
...
//@参数 pPt 接收返回坐标点
//@别名  客户区点到窗口客户区DPI()
	void PointClientToWndClientDPI(POINT* pPt){XEle_PointClientToWndClientDPI(m_hEle, pPt);}
...
//@参数 pRect 接收坐标
//@别名  取窗口客户区坐标DPI()
	void GetWndClientRectDPI(RECT* pRect){XEle_GetWndClientRectDPI(m_hEle, pRect);}
//@备注 获取元素鼠标光标.  
//@返回 返回光标句柄.
//@别名  取光标()
...
//@参数 bEnable 是否启用
//@别名  启用热键前缀()
	void EnableHotkeyPrefix(BOOL bEnable){XBtn_EnableHotkeyPrefix(m_hEle, bEnable);}
//@备注 是否选中状态.  
//@返回 成功返回TRUE否则返回FALSE.
//@别名  是否选中()
...
//@返回 元素句柄.
//@别名  创建扩展()
	HELE CreateEx(int x, int y, int cx, int cy, HXCGUI hParent=NULL){m_hEle = XListBox_CreateEx(x, y, cx, cy, hParent);return m_hEle;}
...
//@参数 bEnable 是否启用
//@别名  启用模板复用()
	void EnableTemplateReuse(BOOL bEnable){XListBox_EnableTemplateReuse(m_hEle, bEnable);}
//@参数 bEnable 是否启用
//@别名  启用虚表()
...
//@别名  置项高度()
	void SetItemHeight(int iItem, int nHeight, int nSelHeight){XListBox_SetItemHeight(m_hEle, iItem, nHeight, nSelHeight);}
//@参数 iItem 项索引
//@参数 pHeight 项高度
//@参数 pSelHeight 项选中时高度
//@别名  取项高度()
	void GetItemHeight(int iItem, int* pHeight, int* pSelHeight){XListBox_GetItemHeight(m_hEle, iItem, pHeight, pSelHeight);}
//@备注 设置选择选.  
//@参数 iItem 项索引.
//@返回 成功返回TRUE否则返回FALSE.
//@别名  置选择项()
...
//@返回 返回项模板句柄
//@别名  取项模板()
	HTEMP GetItemTemplate(){return XListBox_GetItemTemplate(m_hEle);}
//@备注 设置项模板文件.  
//@参数 pStringXML 字符串指针.
//@返回 成功返回TRUE否则返回FALSE.
//@别名  置项模板从字符串()
...
//@返回 成功返回TRUE否则返回FALSE.
//@别名  置项模板从内存()
	BOOL SetItemTemplateXMLFromMem(const void* data, int length){return XListBox_SetItemTemplateXMLFromMem(m_hEle, data, length);}
...
//@返回 成功返回TRUE否则返回FALSE.
//@别名  置项模板从资源ZIP()
	BOOL SetItemTemplateXMLFromZipRes(int id, const wchar_t* pFileName, const wchar_t* pPassword=NULL, HMODULE hModule=NULL){return XListBox_SetItemTemplateXMLFromZipRes(m_hEle, id, pFileName, pPassword, hModule);}
...
//@备注 创建数据适配器并绑定,根据绑定的项模板初始化数据适配器的列(字段名); 
...
//@参数 width 线宽度
//@别名  置拖动矩形颜色()
	void SetDragRectColor(COLORREF color, int width){XListBox_SetDragRectColor(m_hEle, color, width);}
//@备注 排序.  
...
//@返回 元素句柄.
//@别名  创建扩展()
	HELE CreateEx(int x, int y, int cx, int cy, HXCGUI hParent, int col_extend_count){m_hEle = XList_CreateEx(x, y, cx, cy, hParent, col_extend_count);return m_hEle;}
//@备注 增加列.  
//@参数 width 列宽度.
//@返回 返回位置索引.
//@别名  增加列()
...
//@返回 返回插入位置索引.
//@别名  插入列()
	int InsertColumn(int width, int iCol){return XList_InsertColumn(m_hEle, width, iCol);}
//@备注 启用或关闭多选功能.  
//@参数 bEnable 是否启用.
//@别名  启用多选()
...
//@参数 bFull 是否启用铺满.
//@别名  启用行背景铺满()
	void EnableRowBkFull(BOOL bFull){XList_EnableRowBkFull(m_hEle, bFull);}
//@参数 bEnable 是否启用
//@别名  启用固定行高()
	void EnableFixedRowHeight(BOOL bEnable){XList_EnableFixedRowHeight(m_hEle, bEnable);}
//@参数 bEnable 是否启用
//@别名  启用模板复用()
	void EnableTemplateReuse(BOOL bEnable){XList_EnableTemplateReuse(m_hEle, bEnable);}
//@参数 bEnable 是否启用
//@别名  启用虚表()
...
//@参数 nFlags 标志位 @ref list_drawItemBk_flag_.
//@别名  置行背景绘制标志()
	void SetDrawRowBkFlags(int style){XList_SetDrawRowBkFlags(m_hEle, style);}
//@参数 color 颜色值, 请使用宏: RGBA()
//@别名  置分割线颜色()
...
//@返回 成功返回TRUE否则返回FALSE.
//@别名  置项数据()
	BOOL SetItemData(int iRow, int iColumn, vint data){return XList_SetItemData(m_hEle, iRow, iColumn, data);}
//@备注 获取项用户数据.  
//@参数 iRow 行索引.
//@参数 iColumn 列索引.
//@返回 返回用户数据.
//@别名  取项数据()
	vint GetItemData(int iRow, int iColumn){return XList_GetItemData(m_hEle, iRow, iColumn);}
//@备注 设置选择行.  
//@参数 iRow 行索引.
//@返回 成功返回TRUE否则返回FALSE.
//@别名  置选择行()
	BOOL SetSelectRow(int iRow){return XList_SetSelectRow(m_hEle, iRow);}
//@备注 获取选择行.  
//@返回 行索引.
//@别名  取选择行()
	int GetSelectRow(){return XList_GetSelectRow(m_hEle);}
//@备注 获取选择行数量.  
//@返回 返回选择行数量.
//@别名  取选择行数量()
	int GetSelectRowCount(){return XList_GetSelectRowCount(m_hEle);}
//@参数 iRow 行索引
//@返回 成功返回TRUE,否则返回FALSE
//@别名  添加选择行()
	BOOL AddSelectRow(int iRow){return XList_AddSelectRow(m_hEle, iRow);}
//@备注 选择全部行.  
//@别名  置选择全部()
...
//@参数 iRow 行索引.
//@别名  显示指定行()
	void VisibleRow(int iRow){XList_VisibleRow(m_hEle, iRow);}
//@备注 取消选择指定行(这里的项可以理解为行).  
//@参数 iRow 行索引.
//@返回 成功返回TRUE否则返回FALSE.
//@别名  取消选择行()
	BOOL CancelSelectRow(int iRow){return XList_CancelSelectRow(m_hEle, iRow);}
...
//@备注 创建数据适配器，根据绑定的项模板初始化数据适配器的列(字段名);
//数据适配器存储数据, UI对象根据绑定的字段名显示数据适配器中对应的数据;
//例如默认模板是1列, 但是数据有5列,那么列延伸填5
//@参数 colExtend_count 列延伸-预计列表总列数, 默认值0; 限制最大延伸范围, 避免超出范围, 增加不必要的字段.
//@返回 返回适配器句柄.
//@别名  创建数据适配器()
	HXCGUI CreateAdapter(int col_extend_count=3){return XList_CreateAdapter(m_hEle, col_extend_count);}
//@备注 创建数据适配器，根据绑定的项模板初始化数据适配器的列(字段名);
//数据适配器存储数据, UI对象根据绑定的字段名显示数据适配器中对应的数据;
//@返回 返回适配器句柄.
//@别名  列表头创建数据适配器()
...
//例如默认模板是1列, 但是数据有5列,那么列延伸填5
//@参数 col_extend_count 列延伸-预计列表总列数, 默认值0; 限制最大延伸范围, 避免超出范围, 增加不必要的字段.
//@返回 如果成功返回TRUE, 否则返回FALSE.
//@别名  创建数据适配器2()
	BOOL CreateAdapters(int col_extend_count){return XList_CreateAdapters(m_hEle, col_extend_count);}
//@备注 获取数据适配器.  
//@返回 数据适配器句柄.
//@别名  取数据适配器()
...
//@返回 如果成功返回TRUE,否则返回FALSE
//@别名  置项模板从内存()
	BOOL SetItemTemplateXMLFromMem(const void* data, int length){return XList_SetItemTemplateXMLFromMem(m_hEle, data, length);}
...
//@返回 如果成功返回TRUE,否则返回FALSE
//@别名  置项模板从资源ZIP()
	BOOL SetItemTemplateXMLFromZipRes(int id, const wchar_t* pFileName, const wchar_t* pPassword=NULL, HMODULE hModule=NULL){return XList_SetItemTemplateXMLFromZipRes(m_hEle, id, pFileName, pPassword, hModule);}
//@备注 设置项布局模板文件.  
//@参数 pStringXML 字符串指针.
//@返回 成功返回TRUE否则返回FALSE.
//@别名  置项模板从字符串()
...
//@返回 返回列表项模板句柄
//@别名  取项模板()
	HTEMP GetItemTemplate(){return XList_GetItemTemplate(m_hEle);}
//@返回 返回列表头项模板句柄
//@别名  取项模板列表头()
	HTEMP GetItemTemplateHeader(){return XList_GetItemTemplateHeader(m_hEle);}
//@备注 通过模板项ID,获取实例化模板项ID对应的对象句柄.  
//@参数 iRow 行索引.
//@参数 iColumn 列索引.
...
//@别名  取对象所在行()
	int GetRowIndexFromHXCGUI(HXCGUI hXCGUI){return XList_GetRowIndexFromHXCGUI(m_hEle, hXCGUI);}
...
//@别名  取列表头对象所在列()
	int GetHeaderColumnIndexFromHXCGUI(HXCGUI hXCGUI){return XList_GetHeaderColumnIndexFromHXCGUI(m_hEle, hXCGUI);}
//@备注 设置列表头高度.  
//@参数 height 高度.
//@别名  置列表头高度()
...
//@参数 nSelHeight 选中时高度.
//@别名  置行默认高度()
	void SetRowHeightDefault(int nHeight, int nSelHeight){XList_SetRowHeightDefault(m_hEle, nHeight, nSelHeight);}
//@备注 获取行默认高度.  
//@参数 pHeight 高度.
//@参数 pSelHeight 选中时高度.
//@别名  取行默认高度()
	void GetRowHeightDefault(int* pHeight, int* pSelHeight){XList_GetRowHeightDefault(m_hEle, pHeight, pSelHeight);}
//@备注 设置行高度.  
//@参数 iRow 行索引
//@参数 nHeight 高度.
//@参数 nSelHeight 选中时高度.
//@别名  置行高度()
	void SetRowHeight(int iRow, int nHeight, int nSelHeight){XList_SetRowHeight(m_hEle, iRow, nHeight, nSelHeight);}
//@备注 获取行高度.  
//@参数 iRow 行索引
//@参数 pHeight 高度.
//@参数 pSelHeight 选中时高度.
//@别名  取行高度()
	void GetRowHeight(int iRow, int* pHeight, int* pSelHeight){XList_GetRowHeight(m_hEle, iRow, pHeight, pSelHeight);}
//@备注 设置行间距.  
//@参数 nSpace 行间距大小.
//@别名  置行间距()
...
//@参数 width 线宽度
//@别名  置拖动矩形颜色()
	void SetDragRectColor(COLORREF color, int width){XList_SetDragRectColor(m_hEle, color, width);}
//@备注 检测坐标点所在项.  
//@参数 pPt 坐标点.
//@参数 piRow 行索引.
//@参数 piColumn 列索引.
...
//@参数 iRow 行索引.
//@别名  刷新指定行()
	void RefreshRow(int iRow){XList_RefreshRow(m_hEle, iRow);}
//@返回 hEle  元素句柄
//@别名  刷新数据列表头()
	void RefreshDataHeader(){XList_RefreshDataHeader(m_hEle);}
//@备注 删除列.  
//列表头-删除指定列, 列表项保持不变, 数据适配器保持不变, 仅针对列表头项
//@参数 iColumn 列索引.
//@返回 成功返回TRUE否则返回FALSE.
//@别名  删除列()
	BOOL DeleteColumn(int iColumn){return XList_DeleteColumn(m_hEle, iColumn);}
...
//@返回 返回列索引
//@别名  添加列文本2()
	int AddColumnText2(int nWidth, const wchar_t* pText){return XList_AddColumnText2(m_hEle, nWidth, pText);}
//@备注 简化接口  
//首次添加数据时, 需要先创建数据适配器  XList_CreateAdapters() \ 列表_创建数据适配器2()
//@参数 nWidth 列宽度
//@参数 hImage 图片句柄
//@返回 返回列索引
//@别名  添加列图片2()
	int AddColumnImage2(int nWidth, HIMAGE hImage){return XList_AddColumnImage2(m_hEle, nWidth, hImage);}
...
//@返回 行索引
//@别名  添加行文本()
	int AddRowText(const wchar_t* pText){return XList_AddRowText(m_hEle, pText);}
//@备注 首次添加数据时, 需要先创建数据适配器  XList_CreateAdapters() \ 列表_创建数据适配器2()
//@参数 pName 
//@参数 pText 
//@返回 行索引
//@别名  添加行文本扩展()
	int AddRowTextEx(const wchar_t* pName, const wchar_t* pText){return XList_AddRowTextEx(m_hEle, pName, pText);}
//@备注 首次添加数据时, 需要先创建数据适配器  XList_CreateAdapters() \ 列表_创建数据适配器2()
//@参数 hImage 
//@返回 行索引
//@别名  添加行图片()
	int AddRowImage(HIMAGE hImage){return XList_AddRowImage(m_hEle, hImage);}
//@备注 首次添加数据时, 需要先创建数据适配器  XList_CreateAdapters() \ 列表_创建数据适配器2()
//@参数 pName 
//@参数 hImage 
//@返回 行索引
//@别名  添加行图片扩展()
	int AddRowImageEx(const wchar_t* pName, HIMAGE hImage){return XList_AddRowImageEx(m_hEle, pName, hImage);}
//@备注 首次添加数据时, 需要先创建数据适配器  XList_CreateAdapters() \ 列表_创建数据适配器2()
//@参数 iRow 
//@参数 pValue 
//@返回 行索引
//@别名  插入行文本()
	int InsertRowText(int iRow, const wchar_t* pValue){return XList_InsertRowText(m_hEle, iRow, pValue);}
//@备注 首次添加数据时, 需要先创建数据适配器  XList_CreateAdapters() \ 列表_创建数据适配器2()
//@参数 iRow 
//@参数 pName 
//@参数 pValue 
//@返回 行索引
//@别名  插入行文本扩展()
	int InsertRowTextEx(int iRow, const wchar_t* pName, const wchar_t* pValue){return XList_InsertRowTextEx(m_hEle, iRow, pName, pValue);}
//@备注 首次添加数据时, 需要先创建数据适配器  XList_CreateAdapters() \ 列表_创建数据适配器2()
//@参数 iRow 
//@参数 hImage 
//@返回 行索引
//@别名  插入行图片()
	int InsertRowImage(int iRow, HIMAGE hImage){return XList_InsertRowImage(m_hEle, iRow, hImage);}
//@备注 首次添加数据时, 需要先创建数据适配器  XList_CreateAdapters() \ 列表_创建数据适配器2()
//@参数 iRow 
//@参数 pName 
//@参数 hImage 
//@返回 行索引
//@别名  插入行图片扩展()
	int InsertRowImageEx(int iRow, const wchar_t* pName, HIMAGE hImage){return XList_InsertRowImageEx(m_hEle, iRow, pName, hImage);}
//@参数 iRow 行索引
...
//@返回 成功返回TRUE,否则返回FALSE
//@别名  删除行()
	BOOL DeleteRow(int iRow){return XList_DeleteRow(m_hEle, iRow);}
//@备注 数据适配器-删除指定数量的行 和 列表删除指定数量项, 列表头保持不变, 仅针对列表项
//@参数 iRow 
//@参数 nCount 
//@返回 成功返回TRUE,否则返回FALSE
//@别名  删除行扩展()
	BOOL DeleteRowEx(int iRow, int nCount){return XList_DeleteRowEx(m_hEle, iRow, nCount);}
//@备注 数据适配器-删除全部项 和 列表删除全部项, 列表头保持不变, 仅针对列表项
//@别名  删除行全部()
	void DeleteRowAll(){XList_DeleteRowAll(m_hEle);}
...
//@返回 元素句柄.
//@别名  创建扩展()
	HELE CreateEx(int x, int y, int cx, int cy, HXCGUI hParent=NULL){m_hEle = XListView_CreateEx(x, y, cx, cy, hParent);return m_hEle;}
//@备注 创建数据适配器，根据绑定的项模板初始化数据适配器的列(字段名);
//数据适配器存储数据, UI对象根据绑定的字段名显示数据适配器中对应的数据;
//@返回 返回适配器句柄.
//@别名  创建数据适配器()
...
//@返回 如果成功返回TRUE,否则返回FALSE
//@别名  置项模板从内存()
	BOOL SetItemTemplateXMLFromMem(const void* data, int length){return XListView_SetItemTemplateXMLFromMem(m_hEle, data, length);}
...
//@返回 如果成功返回TRUE,否则返回FALSE
//@别名  置项模板从资源ZIP()
	BOOL SetItemTemplateXMLFromZipRes(int id, const wchar_t* pFileName, const wchar_t* pPassword, HMODULE hModule){return XListView_SetItemTemplateXMLFromZipRes(m_hEle, id, pFileName, pPassword, hModule);}
//@备注 置列表项模板.  
...
//@返回 返回项模板句柄
//@别名  取项模板()
	HTEMP GetItemTemplate(){return XListView_GetItemTemplate(m_hEle);}
//@返回 返回项模板组句柄
//@别名  取项模板组()
	HTEMP GetItemTemplateGroup(){return XListView_GetItemTemplateGroup(m_hEle);}
//@备注 通过模板项ID,获取实例化模板项ID对应的对象句柄.  
//@参数 iGroup 组索引.
//@参数 iItem 项索引.
//@参数 nTempItemID 模板项ID.
...
//@参数 bEnable 是否启用
//@别名  启用模板复用()
	void EnableTemplateReuse(BOOL bEnable){XListView_EnableTemplateReuse(m_hEle, bEnable);}
//@参数 bEnable 是否启用
//@别名  启用虚表()
...
//@参数 width 线宽度
//@别名  置拖动矩形颜色()
	void SetDragRectColor(COLORREF color, int width){XListView_SetDragRectColor(m_hEle, color, width);}
//@备注 修改数据后,刷新所有项模板,以便更新数据到模板(如果项可见).  
//@别名  刷新数据()
...
//@返回 元素句柄.
//@别名  创建扩展()
	HELE CreateEx(int x, int y, int cx, int cy, HXCGUI hParent=NULL){m_hEle = XTree_CreateEx(x, y, cx, cy, hParent);return m_hEle;}
//@备注 启用拖动项功能.  
//@参数 bEnable 是否启用.
//@别名  启用拖动项()
...
//@参数 bEnable 是否启用
//@别名  启用模板复用()
	void EnableTemplateReuse(BOOL bEnable){XTree_EnableTemplateReuse(m_hEle, bEnable);}
//@备注 设置项连接线颜色.  
//@参数 color 颜色值, 请使用宏: RGBA()
//@别名  置连接线颜色()
...
//@返回 如果成功返回TRUE,否则返回FALSE
//@别名  置项模板从内存()
	BOOL SetItemTemplateXMLFromMem(const void* data, int length){return XTree_SetItemTemplateXMLFromMem(m_hEle, data, length);}
//@备注 RC资源类型必须为:"RT_RCDATA"
//@参数 id RC资源ID
//@参数 pFileName 文件名
//@参数 pPassword zip密码
...
//@返回 如果成功返回TRUE,否则返回FALSE
//@别名  置项模板从资源ZIP()
	BOOL SetItemTemplateXMLFromZipRes(int id, const wchar_t* pFileName, const wchar_t* pPassword=NULL, HMODULE hModule=NULL){return XTree_SetItemTemplateXMLFromZipRes(m_hEle, id, pFileName, pPassword, hModule);}
//@返回 返回项模板句柄
//@别名  取项模板()
	HTEMP GetItemTemplate(){return XTree_GetItemTemplate(m_hEle);}
//@备注 设置是否绘制指定状态下项的背景.  
//@参数 nFlags 标志位 @ref list_drawItemBk_flag_.
//@别名  置项背景绘制标志()
...
//@返回 返回总行数
//@别名  取总行数扩展()
	int GetRowCountEx(){return XEdit_GetRowCountEx(m_hEle);}
//@备注 包含文本或非文本内容
//@返回 返回数据结构
//@别名  取数据()
...
//@别名  置文本整数()
	...
	CXText GetText(){
		CXText retText;
		int length = XEdit_GetLength(m_hEle);
		if (length > 0)
		{	retText.resize(length);
			int len = XEdit_GetText(m_hEle, (wchar_t*)retText.getPtr(), length + 1);
			retText.resize(len);
		}
		return retText;}//@备注 不包含非文本内容
//@返回 返回临时文本指针, 临时缓存区大小 @ref TEXT_BUFFER_SIZE
//@别名  取文本_临时()
	const wchar_t* GetText_Temp(){return XEdit_GetText_Temp(m_hEle);}
//@备注 获取指定行文本内容
//@参数 iRow 行索引
//@参数 pOut 接收文本内存指针
...
//@参数 iStyle 样式
//@别名  插入文本扩展()
	void InsertTextEx(int iRow, int iCol, const wchar_t* pString, int iStyle){XEdit_InsertTextEx(m_hEle, iRow, iCol, pString, iStyle);}
//@参数 iRow 行索引
//@参数 iCol 列索引
//@参数 hObj 对象句柄
//@别名  插入对象()
	void InsertObject(int iRow, int iCol, HXCGUI hObj){XEdit_InsertObject(m_hEle, iRow, iCol, hObj);}
...
//@返回 成功返回TRUE,否则返回FALSE
//@别名  剪贴板复制()
	BOOL ClipboardCopyAll(){return XEdit_ClipboardCopyAll(m_hEle);}
//@返回 成功返回TRUE,否则返回FALSE
//@别名  剪贴板粘贴()
...
//@别名  插入气泡开始()
	void InsertChatBegin(HIMAGE hImageAvatar, HIMAGE hImageBubble, int nFlag){XEdit_InsertChatBegin(m_hEle, hImageAvatar, hImageBubble, nFlag);}
//@备注 当前行开始
//@参数 hImageAvatar 头像
//@参数 hImageBubble 气泡背景
//@参数 nFlag 标志 @ref chat_flag_
...
//@参数 nWidth 最大宽度
//@别名  置气泡最大宽度()
	void SetChatMaxWidth(int nWidth){XEdit_SetChatMaxWidth(m_hEle, nWidth);}
//@参数 iRow 行索引
//@返回 返回行标识 @ref chat_flag_
//@别名  取指定行气泡标识()
	int GetChatFlags(int iRow){return XEdit_GetChatFlags(m_hEle, iRow);}
};
...
//@返回 如果成功返回TRUE,否则返回FALSE
//@别名  置项模板从内存()
	BOOL SetItemTemplateXMLFromMem(const void* data, int length){return XComboBox_SetItemTemplateXMLFromMem(m_hEle, data, length);}
//@备注 RC资源类型必须为:"RT_RCDATA"
//@参数 id RC资源ID
//@参数 pFileName 文件名
//@参数 pPassword zip密码
...
//@返回 如果成功返回TRUE,否则返回FALSE
//@别名  置项模板从资源ZIP()
	BOOL SetItemTemplateXMLFromZipRes(int id, const wchar_t* pFileName, const wchar_t* pPassword=NULL, HMODULE hModule=NULL){return XComboBox_SetItemTemplateXMLFromZipRes(m_hEle, id, pFileName, pPassword, hModule);}
//@参数 hTemp 项模板句柄
//@返回 如果成功返回TRUE,否则返回FALSE
//@别名  置项模板()
...
//@返回 返回项模板句柄
//@别名  取项模板()
	HTEMP GetItemTemplate(){return XComboBox_GetItemTemplate(m_hEle);}
//@备注 是否绘制下拉按钮.  
//@参数 bEnable 是否绘制.
//@别名  启用绘制下拉按钮()
...
//@返回 返回选择项索引
//@别名  取选择项()
	int GetSelect(){return XMenuBar_GetSelect(m_hEle);}
...
//@备注 获取窗格所属的tabBar, 若是有, 若是没有返回空
//@返回 若是有返回tabBar句柄, 否则返回空
//@别名 取TabBar()
	HELE GetTabBar(){ return XPane_GetTabBar(m_hEle);}
	
//@备注 获取窗格分割条, 分割条类型为基础元素
//@返回 若是有返回分割条句柄, 否则返回空
//@别名 取分割条()
	HELE GetSplitBar(){return XPane_GetSplitBar(m_hEle);}
 
//@备注 获取窗格上的控制按钮
//@参数 number  控制按钮编号,  1: 菜单按钮, 2:停靠码头按钮, 3:关闭按钮
//@返回 返回按钮句柄
//@别名 取按钮()
	HELE GetButton(int number){ return XPane_GetButton(m_hEle, number);}

//@备注 显示隐藏窗格上的控制按钮
//@别名 显示控制按钮()
	void  ShowButton(BOOL bShow){ XPane_ShowButton(m_hEle, bShow); }
...
//@返回 如果成功返回TRUE,否则返回FALSE
//@别名  置进度颜色()
	void SetColorLoad(COLORREF color){XProgBar_SetColorLoad(m_hEle, color);}
//@备注 设置位置点.  
//@参数 pos 位置点.
//@别名  置进度()
...
//@返回 返回插入位置索引.
//@别名  插入分割栏()
	int InsertSeparator(int index=-1, COLORREF color=RGBA(128,128,128,255)){return XToolBar_InsertSeparator(m_hEle, index, color);}
//@备注 启用下拉菜单,显示隐藏的项.  
//@参数 bEnable 是否启用.
//@别名  启用下拉菜单()
...
public:
	//@别名  _句柄
	HXCGUI m_hShape = NULL;
	virtual HXCGUI GetHXCGUI(){ return m_hShape; }
	operator HXCGUI(){ return m_hShape; }
	CXShape(){}
...
public:
	//@别名  _句柄
	HXCGUI m_hAdapter = NULL;
	virtual HXCGUI GetHXCGUI(){ return m_hAdapter; }
	operator HXCGUI(){ return m_hAdapter; }
	CXAdapter(){}
//@返回 返回当前引用计数
//@别名  增加引用计数()
...
//@别名  取项数据类型()
	adapter_date_type_ GetItemDataType(int iRow, int iColumn){return XAdTable_GetItemDataType(m_hAdapter, iRow, iColumn);}
//@备注 获取项数据类型.  
//@参数 iRow 行索引.
//@参数 pName 字段称.
//@返回 返还项数据类型 @ref adapter_date_type_ .
//@别名  取项数据类型扩展()
	adapter_date_type_ GetItemDataTypeEx(int iRow, const wchar_t* pName){return XAdTable_GetItemDataTypeEx(m_hAdapter, iRow, pName);}
//@备注 添加数据列.  
//@参数 pName 字段称.
//@返回 返回列索引.
//@别名  添加列()
...
//@返回 返回行索引值.
//@别名  添加行文本()
	int AddRowText(const wchar_t* pValue){return XAdTable_AddRowText(m_hAdapter, pValue);}
//@备注 添加数据项,填充指定列数据.  
//@参数 pName 字段称.
//@参数 pValue 值.
//@返回 返回行索引.
//@别名  添加行文本扩展()
	int AddRowTextEx(const wchar_t* pName, const wchar_t* pValue){return XAdTable_AddRowTextEx(m_hAdapter, pName, pValue);}
//@备注 添加数据项,默认值放到第一列.  
//@参数 hImage 图片句柄.
//@返回 返回行索引值.
//@别名  添加行图片()
	int AddRowImage(HIMAGE hImage){return XAdTable_AddRowImage(m_hAdapter, hImage);}
//@备注 添加数据项,并填充指定列数据.  
//@参数 pName 字段称.
//@参数 hImage 图片句柄.
//@返回 成功返回TRUE否则返回FALSE.
//@别名  添加行图片扩展()
	int AddRowImageEx(const wchar_t* pName, HIMAGE hImage){return XAdTable_AddRowImageEx(m_hAdapter, pName, hImage);}
//@备注 插入数据项,填充第一列数据.  
//@参数 iRow 插入位置行索引.
//@参数 pValue 值.
//@返回 返回行索引.
//@别名  插入行文本()
	int InsertRowText(int iRow, const wchar_t* pValue){return XAdTable_InsertRowText(m_hAdapter, iRow, pValue);}
//@备注 插入数据项,并填充指定列数据.  
//@参数 iRow 插入位置行索引.
//@参数 pName 字段称.
//@参数 pValue 值.
//@返回 返回行索引.
//@别名  插入行文本扩展()
	int InsertRowTextEx(int iRow, const wchar_t* pName, const wchar_t* pValue){return XAdTable_InsertRowTextEx(m_hAdapter, iRow, pName, pValue);}
//@备注 插入数据项,填充第一列数据.  
//@参数 iRow 插入位置行索引.
//@参数 hImage 图片句柄.
//@返回 返回行索引.
//@别名  插入行图片()
	int InsertRowImage(int iRow, HIMAGE hImage){return XAdTable_InsertRowImage(m_hAdapter, iRow, hImage);}
//@备注 插入数据项,并填充指定列数据.  
//@参数 iRow 插入位置行索引.
//@参数 pName 字段称.
//@参数 hImage 图片句柄.
//@返回 返回行索引.
//@别名  插入行图片扩展()
	int InsertRowImageEx(int iRow, const wchar_t* pName, HIMAGE hImage){return XAdTable_InsertRowImageEx(m_hAdapter, iRow, pName, hImage);}
//@备注 设置项数据.  
//@参数 iRow 行索引.
//@参数 iColumn 列索引.
//@参数 pValue 值.
//@返回 成功返回TRUE否则返回FALSE.
//@别名  置项文本()
	BOOL SetItemText(int iRow, int iColumn, const wchar_t* pValue){return XAdTable_SetItemText(m_hAdapter, iRow, iColumn, pValue);}
//@备注 设置项数据.  
//@参数 iRow 行索引.
//@参数 pName 字段称.
//@参数 pValue 值.
//@返回 成功返回TRUE否则返回FALSE.
//@别名  置项文本扩展()
	BOOL SetItemTextEx(int iRow, const wchar_t* pName, const wchar_t* pValue){return XAdTable_SetItemTextEx(m_hAdapter, iRow, pName, pValue);}
//@备注 设置项数据.  
//@参数 iRow 行索引.
//@参数 iColumn 列索引.
//@参数 nValue 值.
//@返回 成功返回TRUE否则返回FALSE.
//@别名  置项整数值()
	BOOL SetItemInt(int iRow, int iColumn, int nValue){return XAdTable_SetItemInt(m_hAdapter, iRow, iColumn, nValue);}
//@备注 设置项数据.  
//@参数 iRow 行索引.
//@参数 pName 字段称.
//@参数 nValue 值.
//@返回 成功返回TRUE否则返回FALSE.
//@别名  置项整数值扩展()
	BOOL SetItemIntEx(int iRow, const wchar_t* pName, int nValue){return XAdTable_SetItemIntEx(m_hAdapter, iRow, pName, nValue);}
//@备注 设置项数据.  
//@参数 iRow 行索引.
//@参数 iColumn 列索引.
//@参数 fValue 值.
//@返回 成功返回TRUE否则返回FALSE.
//@别名  置项浮点值()
	BOOL SetItemFloat(int iRow, int iColumn, float nValue){return XAdTable_SetItemFloat(m_hAdapter, iRow, iColumn, nValue);}
//@备注 设置项数据.  
//@参数 iRow 行索引.
//@参数 pName 字段称.
//@参数 fValue 值.
//@返回 成功返回TRUE否则返回FALSE.
//@别名  置项浮点值扩展()
	BOOL SetItemFloatEx(int iRow, const wchar_t* pName, float nValue){return XAdTable_SetItemFloatEx(m_hAdapter, iRow, pName, nValue);}
//@备注 设置项数据.  
//@参数 iRow 行索引.
//@参数 iColumn 列索引.
//@参数 hImage 图片句柄.
//@返回 成功返回TRUE否则返回FALSE.
//@别名  置项图片()
	BOOL SetItemImage(int iRow, int iColumn, HIMAGE hImage){return XAdTable_SetItemImage(m_hAdapter, iRow, iColumn, hImage);}
//@备注 设置项数据.  
//@参数 iRow 行索引.
//@参数 pName 字段称.
//@参数 hImage 图片句柄.
//@返回 成功返回TRUE否则返回FALSE.
//@别名  置项图片扩展()
	BOOL SetItemImageEx(int iRow, const wchar_t* pName, HIMAGE hImage){return XAdTable_SetItemImageEx(m_hAdapter, iRow, pName, hImage);}
//@备注 删除行.  
//@参数 iRow 行索引.
//@返回 成功返回TRUE否则返回FALSE.
//@别名  删除行()
	BOOL DeleteRow(int iRow){return XAdTable_DeleteRow(m_hAdapter, iRow);}
//@备注 删除多个项.  
//@参数 iRow 行起始索引.
//@参数 nCount 删除行数量.
//@返回 成功返回TRUE否则返回FALSE.
//@别名  删除行扩展()
	BOOL DeleteRowEx(int iRow, int nCount){return XAdTable_DeleteRowEx(m_hAdapter, iRow, nCount);}
//@备注 删除所有项.  
//@别名  删除行全部()
	void DeleteRowAll(){XAdTable_DeleteRowAll(m_hAdapter);}
//@备注 删除所有列,并且清空所有数据.  
//@别名  删除列全部()
...
//@返回 返回行数量.
//@别名  取行数量()
	int GetCountRow(){return XAdTable_GetCountRow(m_hAdapter);}
//@备注 获取列数量.  
//@返回 返回列数量.
//@别名  取列数量()
...
//@返回 返回文本内容
//@别名  取项文本()
	const wchar_t* GetItemText(int iRow, int iColumn){return XAdTable_GetItemText(m_hAdapter, iRow, iColumn);}
//@备注 获取项文本内容.  
//@参数 iRow 行索引.
//@参数 pName 字段称.
//@返回 返回文本内容
//@别名  取项文本扩展()
	const wchar_t* GetItemTextEx(int iRow, const wchar_t* pName){return XAdTable_GetItemTextEx(m_hAdapter, iRow, pName);}
//@备注 获取项图片句柄.  
//@参数 iRow 行索引.
//@参数 iColumn 列索引.
//@返回 返回图片句柄.
//@别名  取项图片()
	HIMAGE GetItemImage(int iRow, int iColumn){return XAdTable_GetItemImage(m_hAdapter, iRow, iColumn);}
//@备注 获取项图片句柄.  
//@参数 iRow 行索引.
//@参数 pName 字段称.
//@返回 返回图片句柄.
//@别名  取项图片扩展()
	HIMAGE GetItemImageEx(int iRow, const wchar_t* pName){return XAdTable_GetItemImageEx(m_hAdapter, iRow, pName);}
//@备注 获取项值.  
//@参数 iRow 行索引.
//@参数 iColumn 列索引.
//@参数 pOutValue 接收返还值.
//@返回 成功返回TRUE否则返回FALSE.
//@别名  取项整数值()
	BOOL GetItemInt(int iRow, int iColumn, int* pOutValue){return XAdTable_GetItemInt(m_hAdapter, iRow, iColumn, pOutValue);}
//@备注 获取项值.  
//@参数 iRow 行索引.
//@参数 pName 字段称.
//@参数 pOutValue 接收返还值.
//@返回 成功返回TRUE否则返回FALSE.
//@别名  取项整数值扩展()
	BOOL GetItemIntEx(int iRow, const wchar_t* pName, int* pOutValue){return XAdTable_GetItemIntEx(m_hAdapter, iRow, pName, pOutValue);}
//@备注 获取项值.  
//@参数 iRow 行索引.
//@参数 iColumn 列索引.
//@参数 pOutValue 接收返还值.
//@返回 成功返回TRUE否则返回FALSE.
//@别名  取项浮点值()
	BOOL GetItemFloat(int iRow, int iColumn, float* pOutValue){return XAdTable_GetItemFloat(m_hAdapter, iRow, iColumn, pOutValue);}
//@备注 获取项值.  
//@参数 iRow 行索引.
//@参数 pName 字段称.
//@参数 pOutValue 接收返还值.
//@返回 成功返回TRUE否则返回FALSE.
//@别名  取项浮点值扩展()
	BOOL GetItemFloatEx(int iRow, const wchar_t* pName, float* pOutValue){return XAdTable_GetItemFloatEx(m_hAdapter, iRow, pName, pOutValue);}
};
//@备注  继承: CXAdapter, CXBase
//@别名  数据适配器MAP类
...
public:
	//@别名  _句柄
	HMENUX m_hMenu = NULL;
	virtual HXCGUI GetHXCGUI(){ return m_hMenu; }
	operator HXCGUI(){ return m_hMenu; }
	CXMenu(){}
...
//@返回 返回菜单条句柄
//@别名  取菜单条()
	HELE GetMenuBar(){return XMenu_GetMenuBar(m_hMenu);}
//@备注 设置是否自动销毁菜单.  
//@参数 bAuto 是否自动销毁.
//@别名  置自动销毁()
...
public:
	//@别名  _句柄
	HBKM m_hBkM = NULL;
	virtual HXCGUI GetHXCGUI(){ return m_hBkM; }
	operator HXCGUI(){ return m_hBkM; }
	CXBkManager(){}
//@备注 创建背景管理器.  
//@返回 背景管理器句柄.
//@别名  创建()
...
public:
	//@别名  _句柄
	HDRAW m_hDraw = NULL;
	virtual HXCGUI GetHXCGUI(){ return m_hDraw; }
	operator HXCGUI(){ return m_hDraw; }
	CXDraw(){}
//@备注 创建图形绘制模块实例.  
//@参数 hWindow 窗口句柄
//@返回 图形绘制模块实例句柄.
...
//@返回 返回 ID2D1Factory*
//@别名  取D2D工厂()
	vint GetD2dFactory(){return XDraw_GetD2dFactory(m_hDraw);}
//@返回 返回 IDWriteFactory*
//@别名  取D2DWrite工厂()
	vint GetD2dWriteFactory(){return XDraw_GetD2dWriteFactory(m_hDraw);}
//@返回 返回 IWICImagingFactory*
//@别名  取D2DWIC工厂()
	vint GetD2dWICFactory(){return XDraw_GetD2dWICFactory(m_hDraw);}
//@返回 返回 ID2D1RenderTarget*
//@别名  取D2D渲染目标()
	vint GetD2dRenderTarget(){return XDraw_GetD2dRenderTarget(m_hDraw);}
//@返回 返回字体句柄
//@别名  取字体()
	HFONTX GetFont(){return XDraw_GetFont(m_hDraw);}
...
//@参数 y 目标Y坐标.
//@别名  图片遮盖()
	void ImageMask(HIMAGE hImageFrame, HIMAGE hImageFrameMask, RECT* pRect, int x, int y){XDraw_ImageMask(m_hDraw, hImageFrame, hImageFrameMask, pRect, x, y);}
//@备注 使用矩形作为遮罩
//@参数 hImageFrame 图片句柄
//@参数 pRect 矩形坐标
//@参数 pRcMask 遮罩坐标
...
//@别名  图片遮盖矩形()
	void ImageMaskRect(HIMAGE hImageFrame, RECT* pRect, RECT* pRcMask, RECT* pRcRoundAngle){XDraw_ImageMaskRect(m_hDraw, hImageFrame, pRect, pRcMask, pRcRoundAngle);}
//@备注 使用圆形作为遮罩
//@参数 hImageFrame 图片句柄
//@参数 pRect 矩形坐标
//@参数 pRcMask 遮罩坐标
//@别名  图片遮盖圆型()
	void ImageMaskEllipse(HIMAGE hImageFrame, RECT* pRect, RECT* pRcMask){XDraw_ImageMaskEllipse(m_hDraw, hImageFrame, pRect, pRcMask);}
//@参数 pString 字符串.
...
public:
	//@别名  _句柄
	HFONTX m_hFont = NULL;
	virtual HXCGUI GetHXCGUI(){ return m_hFont; }
	operator HXCGUI(){ return m_hFont; }
	CXFont(){}
...
//@返回 返回炫彩字体句柄.
//@别名  创建从内存()
	HFONTX CreateFromMem(const void* data, UINT length, int fontSize=12, int style=fontStyle_regular){m_hFont = XFont_CreateFromMem(data, length, fontSize, style);return m_hFont;}
//@参数 id 资源ID
...
//@返回 返回炫彩字体句柄
//@别名  创建从ZIP()
	HFONTX CreateFromZip(const wchar_t* pZipFileName, const wchar_t* pFileName, const wchar_t* pPassword, int fontSize, int style){m_hFont = XFont_CreateFromZip(pZipFileName, pFileName, pPassword, fontSize, style);return m_hFont;}
//@参数 data 内存块指针
...
//@返回 返回炫彩字体句柄
//@别名  创建从内存ZIP()
	HFONTX CreateFromZipMem(const void* data, int length, const wchar_t* pFileName, const wchar_t* pPassword, int fontSize, int style){m_hFont = XFont_CreateFromZipMem(data, length, pFileName, pPassword, fontSize, style);return m_hFont;}
//@备注 是否自动销毁.  
//@参数 bEnable 是否启用.
//@别名  启用自动销毁()
...
//@别名  置下划线()
	void SetUnderlineEdit(BOOL bUnderline, BOOL bStrikeout){XFont_SetUnderlineEdit(m_hFont, bUnderline, bStrikeout);}
//@备注 仅供edit字体使用, 因为edit不支持下划线字体, 所以需要单独设置
//@参数 bUnderline 接收返回值, 是否启用下划线
//@参数 bStrikeout 接收返回值, 是否启用删除线
//@别名  取下划线()
	void GetUnderlineEdit(BOOL* bUnderline, BOOL* bStrikeout){XFont_GetUnderlineEdit(m_hFont, bUnderline, bStrikeout);}
...
public:
	//@别名  _句柄
	HIMAGE m_hImageSrc = NULL;
	virtual HXCGUI GetHXCGUI(){ return m_hImageSrc; }
	operator HXCGUI(){ return m_hImageSrc; }
	CXImageSrc(){}
//@备注 加载图片从文件.  
//@参数 pFileName 图片文件.
//@返回 图片句柄.
...
//@返回 图片句柄
//@别名  加载从内存ZIP()
	HIMAGE LoadZipMem(const void* data, int length, const wchar_t* pFileName, const wchar_t* pPassword=NULL){m_hImageSrc = XImgSrc_LoadZipMem(data, length, pFileName, pPassword);return m_hImageSrc;}
//@备注 加载流图片,指定区位置及大小.  
//@参数 pBuffer 图片缓冲区
//@参数 nSize 图片缓冲区大小
//@返回 图片句柄.
//@别名  加载从内存()
	HIMAGE LoadMemory(const void* pBuffer, int nSize){m_hImageSrc = XImgSrc_LoadMemory(pBuffer, nSize);return m_hImageSrc;}
//@备注 加载流图片,指定区位置及大小.  
//@参数 pBuffer 图片缓冲区
//@参数 nSize 图片缓冲区大小
//@参数 x 坐标.
...
//@返回 图片句柄.
//@别名  加载从内存指定区域()
	HIMAGE LoadMemoryRect(const void* pBuffer, int nSize, int x, int y, int cx, int cy){m_hImageSrc = XImgSrc_LoadMemoryRect(pBuffer, nSize, x, y, cx, cy);return m_hImageSrc;}
//@备注 加载图片从GDI+的Image对象.  
//@参数 pImage GDI图片对象指针 Bitmap*.
//@返回 成功返回炫彩图片句柄,失败返回FALSE.
//@别名  加载从Image()
	HIMAGE LoadFromImage(const void* pImage){m_hImageSrc = XImgSrc_LoadFromImage(pImage);return m_hImageSrc;}
...
public:
	//@别名  _句柄
	HIMAGE m_hImage = NULL;
	virtual HXCGUI GetHXCGUI(){ return m_hImage; }
	operator HXCGUI(){ return m_hImage; }
	CXImage(){}
//@参数 hImageSrc 图片源句柄
//@返回 返回图片句柄
//@别名  加载从图片源()
...
//@返回 图片句柄
//@别名  加载从内存ZIP()
	HIMAGE LoadZipMem(const void* data, int length, const wchar_t* pFileName, const wchar_t* pPassword=NULL){m_hImage = XImage_LoadZipMem(data, length, pFileName, pPassword);return m_hImage;}
//@备注 RC资源类型必须为:"RT_RCDATA"
//@参数 id RC资源ID
//@参数 pFileName 图片名称
//@参数 pPassword zip压缩包密码
...
//@返回 图片句柄
//@别名  加载从资源ZIP()
	HIMAGE LoadZipRes(int id, const wchar_t* pFileName, const wchar_t* pPassword=NULL, HMODULE hModule=NULL){m_hImage = XImage_LoadZipRes(id, pFileName, pPassword, hModule);return m_hImage;}
//@备注 加载流图片,指定区位置及大小.  
//@参数 pBuffer 图片缓冲区
//@参数 nSize 图片缓冲区大小
//@返回 图片句柄.
//@别名  加载从内存()
	HIMAGE LoadMemory(const void* pBuffer, int nSize){m_hImage = XImage_LoadMemory(pBuffer, nSize);return m_hImage;}
//@备注 加载流图片,指定区位置及大小.  
//@参数 pBuffer 图片缓冲区
//@参数 nSize 图片缓冲区大小
//@参数 x 坐标.
...
//@返回 图片句柄.
//@别名  加载从内存指定区域()
	HIMAGE LoadMemoryRect(const void* pBuffer, int nSize, int x, int y, int cx, int cy){m_hImage = XImage_LoadMemoryRect(pBuffer, nSize, x, y, cx, cy);return m_hImage;}
...
//@返回 图片句柄
//@别名  加载从内存自适应()
	HIMAGE LoadMemoryAdaptive(const void* pBuffer, int nSize, int leftSize, int topSize, int rightSize, int bottomSize){m_hImage = XImage_LoadMemoryAdaptive(pBuffer, nSize, leftSize, topSize, rightSize, bottomSize);return m_hImage;}
//@备注 加载图片从GDI+的Image对象.  
//@参数 pImage GDI图片对象指针 Image*.
//@返回 图片句柄
//@别名  加载从Image()
	HIMAGE LoadFromImage(const void* pImage){m_hImage = XImage_LoadFromImage(pImage);return m_hImage;}
//@备注 加载文件图标,从一个EXE文件或DLL文件或图标文件;例如:*.exe文件的图标.  
//@参数 pFileName 文件名.
//@返回 图片句柄
//@别名  加载文件图标()
...
//@参数 height 高度
//@别名  置缩放大小()
	void SetScaleSize(int width, int height){XImage_SetScaleSize(m_hImage, width, height);}
