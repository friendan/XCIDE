...
public:
	...
	inline wchar_t& operator[](int index) { return (*_text)[index]; }
	inline wchar_t& operator[](UINT index) { return (*_text)[index]; }
...
//@隐藏}
	...
	//@别名  查找(文本)
	int find(const wchar_t* pText)	{return (int)_text->find(pText, 0);}
	...
	//@返回  文本所在位置索引, 失败返回-1
	//@别名  查找2(文本, 开始位置)
	int find2(const wchar_t* pText, int pos){ return (int)_text->find(pText, pos);}
	...
	//@返回  文本所在位置索引, 字符为单位, 失败返回-1
	//@别名  反向查找(文本)
	int rfind(const wchar_t* pText) { return (int)_text->rfind(pText); }
	...
	//@返回  文本所在位置索引, 失败返回-1
	//@别名  反向查找2(文本, 开始位置)
	int rfind2(const wchar_t* pText, int pos = -1) { return (int)_text->rfind(pText, pos); }
	...
	//@返回  末尾字符
	//@别名  取尾字符()
	wchar_t back() { return _text->back(); }
	...
	//@返回  首字符
	//@别名  取首字符()
	wchar_t front() {	return _text->front(); }
	...
	//@返回 返回指定位置的字符
	//@别名 取字符(位置索引)
	const wchar_t at(int pos) { return (*_text)[pos]; }
private:
...
public:
	...
	inline char& operator[](int  index) { return (*_text)[index]; }
	inline char& operator[](UINT index) { return (*_text)[index]; }
...
//@隐藏}
	...
	//@返回  文本所在位置索引, 字符为单位, 失败返回-1
	//@别名  查找(文本)
	int find(const char* pText) { return (int)_text->find(pText, 0); }
	...
	//@返回  文本所在位置索引, 失败返回-1
	//@别名  查找2(文本, 开始位置)
	int find2(const char* pText, int pos) { return (int)_text->find(pText, pos); }
	...
	//@返回  文本所在位置索引, 字符为单位, 失败返回-1
	//@别名  反向查找(文本)
	int rfind(const char* pText) { return (int)_text->rfind(pText); }
	...
	//@返回  文本所在位置索引, 失败返回-1
	//@别名  反向查找2(文本, 开始位置)
	int rfind2(const char* pText, int pos = -1) { return (int)_text->rfind(pText, pos); }
...
//@隐藏}
	...
	//@参数  成员数量
	//@别名  置大小(数量)
	void resize(int count) { _list->resize(count); }
//@隐藏{
	//@备注  与 getSize() 功能相同
...
//@示例 	文本型A 结果 = 文本组合(A"abc", 123, 6.5f) //结果为: A"abc1236.5"
//@返回 组合后的文本
//@别名 文本组合A()
CXTextA xcl_mkStrA(...);
