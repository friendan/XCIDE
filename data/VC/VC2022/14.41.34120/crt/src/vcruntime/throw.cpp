...
#if _MSC_FULL_VER >= 193826504 // TRANSITION, toolset update
noexcept(false)
#endif // TRANSITION, toolset update
{
    ...
    void *throwImageBase = nullptr;
    if (pTI) {
        throwImageBase = RtlPcToFileHeader(const_cast<void *>(static_cast<const void *>(pTI)), &throwImageBase);
    }
