...
private:
	...
	[[nodiscard]] static bool increment_buffer(size_t count)
	{
		DASSERT(count > 0);
	...
	// by one character.
	[[nodiscard]] static char get_current_character_and_increment_buffer()
	{
		char c = *gName;
		if (c) ++gName;
		return c;
...
DName UnDecorator::getFloatingPoint(int type_category)
{
	...
	if (auto result = getValue())
	{
		...
		{
			return get_encoding<double>(value);
		}
		else if (type_category == TC_float)
		{
			return get_encoding<float>(value);
		}
	}
