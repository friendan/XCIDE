...
/// </summary>
/// <param name="policy">
///     A const reference to the scheduler policy.
/// </param>
/// <returns>
///     A pointer to the new scheduler
/// </returns>
Scheduler* Scheduler::Create(const SchedulerPolicy& policy)
{
...
///     scheduler exists. Once a default policy is set, it remains in effect until the next valid call to the API.
/// </summary>
/// <param name="_Policy">
///     The policy to be set as the default. The runtime will make a copy of the policy for its use, and the user
///     is responsible for the lifetime of the policy that is passed in.
/// </param>
void Scheduler::SetDefaultSchedulerPolicy(const SchedulerPolicy & _Policy)
{
...
namespace details
{
    ...
    void SchedulerBase::Initialize()
    {
        ...
        m_pCoreAffinityQuickCache = _concrt_new ScheduleGroupSegmentBase* [static_cast<size_t>(ridCounter) << QUICKCACHEPAD_SHIFT];
    ...
    ExternalContextBase * SchedulerBase::AttachExternalContext(bool explicitAttach)
    {
        ...
        if (pCurrentContext != NULL)
        {
            ...
            // TLS values are setup, relies on the current context in TLS, it might behave non-deterministically.
            // e.g. Creating an external context looks at the anonymous schedule group - since the current
...
#pragma warning(pop)
    ...
    void SchedulerBase::VirtualProcessorIdle(bool fIdle)
    {
        ...
        if (fIdle)
        {
            ...
            // if the shutdown in progress bit flag is set.
            //
            if (((val & GATE_COUNT_MASK) == 0) && ((val & SHUTDOWN_INITIATED_FLAG) != 0))
            {
                // The suspend flag should not bet set.
    ...
    void SchedulerBase::PhaseTwoShutdown()
    {
        ...
        // all active virtual processors are idle. However, it is possible that a foreign thread is trying to activate a virtual processor
    ...
    void SchedulerBase::RemoveVirtualProcessors(IVirtualProcessorRoot **ppVirtualProcessorRoots, unsigned int count)
    {
        ...
        for (unsigned int i = 0; i < count; ++i)
        {
            ...
            {
                // We must synchronize with a potential RemoveVirtualProcessor for this virtual processor due to the RM taking the underlying
    ...
    void SchedulerBase::InvokeOnSafePoint(SafePointInvocation *pInvocation)
    {
        ...
        // scan the entire list.  In order to guarantee this, the increment and the list addition must be atomic with respect to each other.  Right
    ...
    ///     after the call is made.  This is an intrusive invocation with the current SafePointInvocation class incurring no heap
    ///     allocations.
    /// </summary>
    ...
    SchedulingNode *SchedulerBase::FindCurrentNode()
    {
        ...
        {
            InternalContextBase *pInternalContext = static_cast<InternalContextBase *>(pCurrentContext);
            pInternalContext->EnterCriticalRegion();
            pCurrentNode = pInternalContext->m_pVirtualProcessor->GetOwningNode();
            pInternalContext->ExitCriticalRegion();
        }
