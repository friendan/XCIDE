...
namespace details
{
    ...
    void ResourceManager::InitializeSystemInformation(bool fSaveTopologyInfo)
    {
...

        {
            ...
            // will be incremented for every processor group.
        ...
        // Win7 or higher has a PSYSTEM_LOGICAL_PROCESSOR_INFORMATION_EX structure to support > 64 cores
        {
            ...
            while (byteOffset < s_logicalProcessorInformationLength)
            {
                switch (pSysInfoEx->Relationship)
                {
                    case RelationProcessorPackage:
                        {
                            // If this machine had more packages (actually groups) than NUMA nodes, we should create one scheduling node per package (actually group).
                            if (s_countProcessorPackages)
                            {
                                ASSERT(pSysInfoEx->Processor.GroupCount >= 1);
                                for (int group = 0; group < pSysInfoEx->Processor.GroupCount; ++group)
                                {
                                    if (pSysInfoEx->Processor.GroupMask[group].Mask != 0)
                                    {
#pragma warning(push)
#pragma warning(disable: 6385) // TRANSITION, VSO-1806041
                                        // This index is valid due to ResourceManager::InitializeSystemInformation() above.
                                        // Below, we assert that the final value of the index is equal to the count.
                                        _Analysis_assume_(nodeIndex < m_nodeCount);
...
#pragma warning(pop)
                    ...
                    case RelationNumaNode:
                        {
                            // If this machine had more NUMA nodes than packages (actually groups), we should create one scheduling node per NUMA node.
                            if (!s_countProcessorPackages)
                            {
                            ...
                            if (pSysInfoEx->NumaNode.GroupMask.Mask != 0)
                            {
                                // This index is valid due to ResourceManager::InitializeSystemInformation() above.
                                // Below, we assert that the final value of the index is equal to the count.
                                _Analysis_assume_(numaIndex < m_numaNodeCount);
        ...
        //
        // If we count processor packages (actually groups), go back and map them to the NUMA information.
        //
        if (s_countProcessorPackages)
        {
